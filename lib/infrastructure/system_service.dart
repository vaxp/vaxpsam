...
class SystemService {
  ...
  // Advanced Debugging & Troubleshooting Methods
  ...
  Stream<CommandOutputLine> getGPUInfo() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Retrieving GPU information...',
      isError: false,
    );
    ...
    try {
      await for (final line in runAsRoot(['nvidia-smi'])) {
        yield line;
      }
    } catch (_) {
      yield CommandOutputLine(
    ...
    try {
      await for (final line in runAsRoot(['glxinfo', '|', 'grep', 'OpenGL'])) {
        yield line;
      }
    } catch (_) {
      yield CommandOutputLine(
        timestamp: DateTime.now(),
        text: 'glxinfo not available',
        isError: false,
      );
    }
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'GPU information retrieval completed',
      isError: false,
    );
  }

  /// Gets GPU usage information
  Stream<CommandOutputLine> getGPUUsage() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Monitoring GPU usage...',
      isError: false,
    );
    ...
    try {
      ...
      yield CommandOutputLine(
        timestamp: DateTime.now(),
        text: 'nvidia-smi not available - trying alternative methods',
        isError: false,
      );
    }
    ...
    try {
      await for (final line in runAsRoot(['radeontop', '-d', '1'])) {
        yield line;
      }
    } catch (_) {
      yield CommandOutputLine(
        timestamp: DateTime.now(),
        text: 'radeontop not available',
        isError: false,
      );
    }
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'GPU usage monitoring completed',
      isError: false,
    );
  }

  /// Gets VRAM usage information
  Stream<CommandOutputLine> getVRAMUsage() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Checking VRAM usage...',
      isError: false,
    );
    ...
    try {
      ...
      yield CommandOutputLine(
        timestamp: DateTime.now(),
        text: 'VRAM monitoring not available for this GPU',
        isError: false,
      );
    }
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'VRAM usage check completed',
      isError: false,
    );
  }

  /// Gets GPU temperature
  Stream<CommandOutputLine> getGPUTemperature() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Checking GPU temperature...',
      isError: false,
    );
    ...
    try {
      ...
      yield CommandOutputLine(
        timestamp: DateTime.now(),
        text: 'GPU temperature monitoring not available',
        isError: false,
      );
    }
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'GPU temperature check completed',
      isError: false,
    );
  }

  /// Installs NVIDIA GPU monitoring tools
  Stream<CommandOutputLine> installNvidiaTools() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Installing NVIDIA GPU monitoring tools and drivers...',
      isError: false,
    );
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'NVIDIA tools and drivers installation completed',
      isError: false,
    );
  }

  /// Installs AMD GPU monitoring tools
  Stream<CommandOutputLine> installAmdTools() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Installing AMD GPU monitoring tools...',
      isError: false,
    );
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'AMD tools installation completed',
      isError: false,
    );
  }

  /// Installs Intel GPU monitoring tools
  Stream<CommandOutputLine> installIntelTools() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Installing Intel GPU monitoring tools...',
      isError: false,
    );
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Intel tools installation completed',
      isError: false,
    );
  }

  /// Installs general GPU monitoring tools
  Stream<CommandOutputLine> installGeneralGpuTools() async* {
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'Installing general GPU monitoring tools...',
      isError: false,
    );
    ...
    yield CommandOutputLine(
      timestamp: DateTime.now(),
      text: 'General GPU tools installation completed',
      isError: false,
    );
  }

  /// Checks if NVIDIA tools are available
  Future<bool> isNvidiaToolsAvailable() async {
    try {
  ...
  /// Checks if AMD tools are available
  Future<bool> isAmdToolsAvailable() async {
    try {
      final result = await Process.run('which', ['radeontop']);
      return result.exitCode == 0;
    } catch (_) {
      return false;
    }
  ...
  /// Checks if Intel tools are available
  Future<bool> isIntelToolsAvailable() async {
    try {
      final result = await Process.run('which', ['intel_gpu_top']);
      return result.exitCode == 0;
    } catch (_) {
      return false;
    }
  ...
  Future<Map<String, dynamic>> getParsedGPUUsage() async {
    ...
    try {
        ...
        if (result.exitCode == 0) {
          ...
          if (lines.isNotEmpty) {
            ...
            if (parts.length >= 6) {
              ...
              data['utilization'] = int.tryParse(parts[1].trim()) ?? 0;
              data['memoryUsed'] = int.tryParse(parts[2].trim()) ?? 0;
      ...
      // Try AMD if NVIDIA not available
      else if (await isAmdToolsAvailable()) {
        final result = await Process.run('radeontop', ['-d', '1', '-l', '1']);
        if (result.exitCode == 0) {
          ...
          for (final line in lines) {
            ...
            if (line.contains('Temp') || line.contains('°C')) {
              final match = RegExp(r'(\d+)°?C').firstMatch(line);
              if (match != null) {
                data['temperature'] = int.tryParse(match.group(1)!) ?? 0;
              }
            }
          }
          ...
          data['powerDraw'] = 0.0;
          data['memoryPercent'] = data['memoryPercent'] ?? 0;
        }
      }
      // Try Intel GPU tools
      else if (await isIntelToolsAvailable()) {
        final result = await Process.run('sudo', ['intel_gpu_top', '-s', '1']);
        if (result.exitCode == 0) {
          final output = result.stdout.toString();
          data['vendor'] = 'intel';
          data['name'] = 'Intel GPU';
          ...
          for (final line in lines) {
            ...
            // Look for memory usage
            if (line.contains('Memory') || line.contains('VRAM')) {
              final match = RegExp(r'(\d+)/(\d+)').firstMatch(line);
              if (match != null) {
                data['memoryUsed'] = int.tryParse(match.group(1)!) ?? 0;
                data['memoryTotal'] = int.tryParse(match.group(2)!) ?? 0;
      ...
      // Fallback to basic info using lspci
      else {
        final result = await Process.run('bash', ['-c', 'lspci | grep -i vga']);
        if (result.exitCode == 0) {
          final output = result.stdout.toString().trim();
          data['vendor'] = 'unknown';
          data['name'] = output.isNotEmpty ? output : 'Unknown GPU';
          data['utilization'] = 0;
          data['memoryUsed'] = 0;
          data['memoryTotal'] = 0;
          data['temperature'] = 0;
          data['powerDraw'] = 0.0;
          data['memoryPercent'] = 0;
        } else {
          data['vendor'] = 'error';
          data['error'] = 'No GPU detected or tools not available';
        }
      }
    } catch (e) {
      data['vendor'] = 'error';
